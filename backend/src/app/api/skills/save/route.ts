...
export const POST = withAuth(async (req, { auth }) => {
  ...
  if (!body?.id || !body.groups)
    return apiError("bad_request", "Translation payload required.");
  ...
  const updatedPersona = generatePersona({
    ...
    explore: {
      likedRoleIds: summary.likedRoleIds,
      dislikedRoleIds: summary.dislikedRoleIds,
    },
    skillTranslations: list.map((t) => ({
  ...
  await db.insertSkillTranslation(auth.userId, body);
  await db.upsertPersona(auth.userId, updatedPersona);
